...
const TEMAS = {
  ...
  notte: [
    ...
    ["Il mistero dello spazio mi affascina profondamente.", "affascina", "O misterio do espaco me fascina profondamente.", "verbo affascinare: attrarre con forza"],
  ...
  tristezza: [
    ...
    ["Cerco nel silenzio una risposta che non arriva.", "risposta", "Procuro no silencio uma risposta che nao chega.", "sostantivo: cio che risponde"],
  ...
  vita: [
    ...
    ["La vita e piu bella quando la condividi con gli altri.", "condividi", "A vita e mais bonita quando a compartilhas com os outros.", "verbo condividere: dividere con altri"],
...
const TEMA_MAP = {
  amore: 'amore', estate: 'estate', notte: 'notte', ballo: 'ballo',
  tristezza: 'tristezza', viaggio: 'viaggio', amicizia: 'amicizia',
  ribellione: 'ribellione', nostalgia: 'nostalgia', vita: 'vita',
};

function makeEstrofa(idx, [testo, parola, traducao, dica]) {
  return {
    id: idx,
    texto_completo: testo,
    texto_lacuna: testo.replace(parola, '___'),
...
function isGeneric(estrofes) {
  return estrofes.some(e => e.texto_completo && e.texto_completo.includes('Questo') && e.texto_completo.includes('verso'));
}

const dataRaw = fs.readFileSync('data/canzoni.json', 'utf-8');
const data = JSON.parse(dataRaw);
const songs = data.canzoni;

const temaUsage = {};
for (const k of Object.keys(TEMAS)) temaUsage[k] = 0;

let fixed = 0, skipped = 0, unknown = [];

for (const song of songs) {
  const m = song.id.match(/can_(\d+)/);
  if (!m) continue;
  const num = parseInt(m[1]);
  if (num < 51 || num > 150) continue;

  const tema = song.tema || '';
  const tk = TEMA_MAP[tema];
  if (!tk) { unknown.push([song.id, tema]); continue; }

  if (!isGeneric(song.estrofes || [])) { skipped++; continue; }

  const pool = TEMAS[tk];
  const start = temaUsage[tk];
  const frases = [0,1,2,3].map(i => pool[(start + i) % pool.length]);
  temaUsage[tk] += 4;

  song.estrofes = frases.map((f, i) => makeEstrofa(i + 1, f));
  song.vocabulario_chave = frases.map(f => f[1]);
  fixed++;
}
...
console.log('Saved data/canzoni.json');

// Quick verify
const samples = ['can_051', 'can_057', 'can_060', 'can_147', 'can_150'];
for (const s of data.canzoni) {
  if (samples.includes(s.id)) {
    console.log(`\n${s.id} [${s.tema}]: ${s.titulo}`);
    if (s.estrofes && s.estrofes[0]) {
      console.log('  Estrofe 1:', s.estrofes[0].texto_completo);
    }
  }
}
